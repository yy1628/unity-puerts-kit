...
class UnityXMLHttpRequest extends XMLHttpRequestBase {
	...
	private _internal_respons_headers: System.Collections.Generic.Dictionary$2<
		string,
		string
	>;
	...
	send(body?: BodyInit | null): void {
		...
		switch (this._method) {
			case "PUT":
				this._request = UnityEngine.Networking.UnityWebRequest.Put(
					this._url.url,
					this._request_body
				);
				break;
			case "POST":
				...
				break;
			case "GET":
				this._request = UnityEngine.Networking.UnityWebRequest.Get(
					this._url.url
				);
				break;
			case "DELETE":
				this._request = UnityEngine.Networking.UnityWebRequest.Delete(
					this._url.url
				);
				break;
			default:
				this._request = new UnityEngine.Networking.UnityWebRequest(
					this._url.url,
					this._method
				);
				break;
		...
		if (UnityEngine.Networking.UnityWebRequest.certificateHandler) {
			...
			this._request.certificateHandler =
				UnityEngine.Networking.UnityWebRequest.certificateHandler;
		}
		for (let key of Object.getOwnPropertyNames(this._request_headers)) {
			const value = this._request_headers[key];
	...
	public $tick() {
		if (this._request) {
			...
			this._status =
				Number(this._request.responseCode) || HttpStatusCode.Continue;
			if (this.readyState === XMLHttpRequestReadyState.OPENED) {
				this._internal_respons_headers =
					this._request.GetResponseHeaders();
				if (
					this._internal_respons_headers &&
					this._internal_respons_headers.Count
				) {
	...
	protected $process_response() {
		...
		switch (this.responseType) {
			...
			case "json":
				...
				break;
			default:
				this._response = this._request.downloadHandler
					? this._request.downloadHandler.data
					: null;
				break;
		}
	}
